...
The premium sub-brand inside Nexus Studio. PHX Growth flies a brand's paid media
...
in `app/phx-growth/*` and `components/phx-growth/*` and imports everything from
here via the `@/phx-growth` barrel.
...
| `index.ts` | Barrel + `PHX_GROWTH` brand constants. |
...
| Route | Does |
| --- | --- |
| `GET /api/phx-growth/connectors/[provider]` | Mints a live OAuth consent URL (302) or a legible 503 when creds are missing. Sets a CSRF `state` cookie. |
| `POST /api/phx-growth/higgsfield` | Fires a Higgsfield render for the Creative Forge. |
| `POST /api/phx-growth/workflows/[id]` | Triggers an n8n workflow webhook (dry-run receipt when `N8N_BASE_URL` is unset). |
| `GET /api/phx-growth/cron/autopilot` | **Vercel cron, every 15 min** — the 24/7 optimization heartbeat. Gated by `CRON_SECRET`. |
| `GET /api/phx-growth/cron/nurture` | **Vercel cron, hourly** — lead re-engagement sweep. Gated by `CRON_SECRET`. |
| `GET /api/phx-growth/autopilot` | Public, secret-free read of the heartbeat: last sweep, flight-log tail, total sweeps. |
