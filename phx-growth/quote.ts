...
//      PHX_GROWTH_FROM_EMAIL    optional — verified sender, e.g. "PHX Growth <hello@yourdomain.com>"
//      PHX_GROWTH_OWNER_EMAIL   optional — where the internal lead alert goes
...
function normalizeQuote(q: Quote): Quote {
  ...
  const addOns = (Array.isArray(q.addOns) ? q.addOns : [])
    ...
    .slice(0, 2);
  return {
    subject: typeof q.subject === 'string' && q.subject.trim() ? q.subject : 'Your PHX Growth flight plan and price',
...
export async function draftQuote(lead: LeadInput): Promise<Quote> {
  if (aiConfigured()) {
    try {
      return normalizeQuote(await askClaudeJSON<Quote>({
        system:
          'You are PHX Growth, an autonomous AI media buyer, replying to a new inbound lead. ' +
          ...
          '3-5 short paragraphs; sign off as "PHX Growth Flight Command".\n\nCATALOG:\n' + CATALOG +
  ...
  return {
    subject: `${first} — your PHX Growth flight plan and price`,
    paragraphs: [
      `Hi ${first}, thanks for reaching out${lead.company ? ` about ${lead.company}` : ''}. Your goal — ${lead.goal || 'more customers'} — is exactly what PHX Growth is built to fly.`,
...
function renderHtml(q: Quote): string {
  ...
  return (
    ...
    `<div style="font-size:13px;letter-spacing:.2em;text-transform:uppercase;background:linear-gradient(90deg,#00D4FF,#6C63FF,#FF2E9A);-webkit-background-clip:text;background-clip:text;color:transparent;font-weight:600;margin-bottom:20px">PHX Growth</div>` +
    paras +
...
async function sendEmail(to: string, subject: string, html: string, replyTo?: string): Promise<string> {
  ...
  const from = process.env.PHX_GROWTH_FROM_EMAIL || 'PHX Growth <onboarding@resend.dev>';
...
export async function sendQuoteEmail(lead: LeadInput, quote: Quote): Promise<string> {
  return sendEmail(lead.email, quote.subject, renderHtml(quote), process.env.PHX_GROWTH_OWNER_EMAIL);
}
...
export function draftFollowUp(lead: LeadInput, planName: string, touch: 1 | 2): Quote {
  ...
  return {
    subject:
      touch === 1
        ? `${first} — did your PHX Growth flight plan land?`
...
export async function sendFollowUpEmail(lead: LeadInput, planName: string, touch: 1 | 2): Promise<string> {
  ...
  return sendEmail(lead.email, q.subject, renderHtml(q), process.env.PHX_GROWTH_OWNER_EMAIL);
}
...
export async function sendOwnerAlert(lead: LeadInput, quote: Quote): Promise<string | null> {
  const owner = process.env.PHX_GROWTH_OWNER_EMAIL;
