...
// instance, plus a webhook path PHX Growth triggers at runtime. These are the
// looping brains behind the services — the parts that run while nobody watches.
//
// Trigger flow: the app POSTs to `/api/phx-growth/workflows/[id]` which forwards
...
export const WORKFLOWS: Workflow[] = [
  {
    ...
    summary:
      'Every 15 minutes: pull spend and real profit across every channel, rank by marginal return, and move budget to the leaders — before the platforms notice.',
    trigger: 'schedule',
  ...
  },
  {
    ...
    nodes: [
      ...
      { name: 'Forge Render', type: 'n8n-nodes-base.httpRequest', notes: 'Generate reels' },
    ...
    connections: {
      ...
      'Compose Brief': ['Forge Render'],
      'Forge Render': ['Wait for Render'],
  ...
  },
  {
    ...
    nodes: [
      ...
      { name: 'Forge Creative', type: 'n8n-nodes-base.httpRequest', notes: 'First ad batch' },
      ...
      { name: 'Notify Client', type: 'n8n-nodes-base.slack', notes: 'Wheels up' },
    ],
    connections: {
      'Product URL In': ['Scrape + Research'],
      'Scrape + Research': ['Build Personas'],
      'Build Personas': ['Draft Strategy'],
      'Draft Strategy': ['Forge Creative'],
      'Forge Creative': ['Wire Tracking'],
  ...
  },
  {
    ...
    serviceId: 'cross-channel',
    summary:
      'When a HubSpot deal changes stage (esp. → closed-won), read the deal, contact and stored click ids, then push an offline conversion carrying the real deal value back to Meta CAPI, Google Ads and TikTok — so the platforms optimize on revenue that actually closed. Idempotent, retried, dry-run aware, logged to Slack.',
    trigger: 'event',
  ...
  },
  {
    ...
    serviceId: 'cross-channel',
    summary:
      'Daily, turn HubSpot lifecycle + deal truth into three ad audiences — suppress customers and closed-won from prospecting, retarget stalled deals, seed lookalikes from real buyers — then sync them (hashed, consent-aware) to Meta/Google/TikTok behind an approval gate.',
    trigger: 'schedule',
    cadence: 'Daily',
