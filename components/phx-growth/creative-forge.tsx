...
// ─── PHX GROWTH · CREATIVE FORGE (client) ───────────────────────────────────
// The live, on-site demo of the product automating itself: a visitor types a
// brand + product, picks a vibe/channel, and PHX Growth fires Higgsfield to
// forge an ad reel — streaming the render as it "builds". Talks to
// POST /api/phx-growth/higgsfield. Works with or without a real Higgsfield key
// (the route returns a shaped simulated job when none is set).

import { useEffect, useRef, useState } from "react";
import { VIBES, getShowcase, type CreativeJob, type CreativeVibe } from "@/phx-growth";
...
export function CreativeForge() {
  ...
  async function forge() {
    ...
    try {
      const res = await fetch("/api/phx-growth/higgsfield", {
        method: "POST",
        ...
        {!job && !busy && (
          <div className="relative flex flex-col items-center gap-4 px-8 py-8">
            ...
            <div className="text-text-secondary text-sm max-w-xs mx-auto text-center">
              A real PHX Growth reel. Fill in a brand, pick a vibe, and forge your own.
            </div>
          </div>
        )}
        ...
        {job && !busy && (() => {
          ...
          <div className="relative w-full px-8 py-8 space-y-4">
            ...
            <div className="max-w-sm mx-auto grid grid-cols-2 gap-2 text-center text-[11px]">
              ...
              <Stat label="Source" value={live ? "PHX Growth · live" : "PHX Growth · showcase"} />
            </div>
