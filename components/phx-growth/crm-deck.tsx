...
// ─── PHX GROWTH · ORBITAL CRM DECK ──────────────────────────────────────────
// The client command surface. Talks to /api/phx-growth/crm/* — the roster is
...
import {
  ...
  type StageId,
  type ClientSource,
} from "@/phx-growth/crm-shared";
import { PIXEL_AGENTS } from "@/phx-growth/agents";
...
export function CrmDeck() {
  ...
  const refresh = useCallback(async () => {
    try {
      const res = await fetch("/api/phx-growth/crm/clients", { cache: "no-store" });
  ...
  useEffect(() => {
    refresh();
    fetch("/api/phx-growth/crm/integrations", { cache: "no-store" })
      .then((r) => r.json())
  ...
  const patchClient = useCallback(
    async (id: string, patch: Record<string, unknown>) => {
      await fetch(`/api/phx-growth/crm/clients/${id}`, {
        method: "PATCH",
  ...
  const removeClient = useCallback(
    async (id: string) => {
      await fetch(`/api/phx-growth/crm/clients/${id}`, { method: "DELETE" }).catch(() => {});
      setExpanded(null);
      await refresh();
    },
    [refresh]
    ...
    <header className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.02] p-10 backdrop-blur-md">
      ...
      <div className="relative space-y-3">
        ...
        <p className="max-w-2xl text-text-secondary">
          The roster PHX Growth flies for — tracked from first radar blip to permanent orbit, wired into the
...
function AcquisitionDock({
  ...
  const submit = async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/phx-growth/crm/clients", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: form.name,
