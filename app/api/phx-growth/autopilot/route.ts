// ─── PHX GROWTH · AUTOPILOT STATUS ──────────────────────────────────────────
// GET /api/phx-growth/autopilot
...
import { autopilotStatus } from '@/phx-growth/cron';

export const dynamic = 'force-dynamic';

export async function GET() {
  const status = await autopilotStatus();
  return NextResponse.json({ ok: true, service: 'phx-growth', autopilot: status });
}
