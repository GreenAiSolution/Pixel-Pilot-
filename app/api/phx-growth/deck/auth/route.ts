// ─── PHX GROWTH · DECK AUTH ─────────────────────────────────────────────────
// POST   /api/phx-growth/deck/auth → { key } — exchange the operator key for
//        the httpOnly deck cookie (30 days). Rate-limited hard: it's a lock.
// DELETE /api/phx-growth/deck/auth → clear the cookie (log out).
// GET    /api/phx-growth/deck/auth → { access } — safe status probe.

import { NextRequest, NextResponse } from 'next/server';
import { guard, ok, fail, requestId, log } from '@/phx-growth/api';
import {
  DECK_COOKIE,
  deckAccess,
  deckCookieValue,
