// ─── PHX GROWTH · CRM CLIENT (single) ───────────────────────────────────────
// GET    /api/phx-growth/crm/clients/:id → one client record
// PATCH  /api/phx-growth/crm/clients/:id → edit fields, advance stage,
//        link/unlink integrations, log timeline events (health recomputed)
// DELETE /api/phx-growth/crm/clients/:id → remove from the roster
// Stage advances and integration changes fan out through Zapier so the rest of
// the client's stack hears about them.

import { NextRequest, NextResponse } from 'next/server';
import { guard, ok, fail, requestId } from '@/phx-growth/api';
import {
  getClient,
  updateClient,
  deleteClient,
...
import { fireZapier } from '@/phx-growth/executor';
