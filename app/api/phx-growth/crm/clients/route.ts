// ─── PHX GROWTH · CRM CLIENTS ───────────────────────────────────────────────
// GET  /api/phx-growth/crm/clients → roster + command summary
// POST /api/phx-growth/crm/clients → acquire a client and wire the engine:
...
import { fireZapier, triggerWorkflow, type Receipt } from '@/phx-growth/executor';
import { isConnected as qbConnected, createCustomer as qbCreateCustomer } from '@/phx-growth/quickbooks';
    ...
    try {
      if (await qbConnected()) {
        const customer = await qbCreateCustomer({
          ...
          email: client.email,
          phone: client.phone,
          notes: `PHX Growth CRM · ${client.company} · ${client.id}`,
        });
        receipts.push({
          target: 'quickbooks',
          configured: true,
        ...
        receipts.push({
          ...
          configured: false,
          ok: false,
          detail: 'QuickBooks not connected — connect at /api/phx-growth/connectors/quickbooks',
        });
      }
    } catch (err) {
      receipts.push({
