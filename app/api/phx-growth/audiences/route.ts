// ─── PHX GROWTH · AUDIENCE COMPUTE / COMMIT ─────────────────────────────────
// GET  /api/phx-growth/audiences?portalId=…  → hashed membership deltas + verdict
// POST /api/phx-growth/audiences             → { portalId, runId, decision }
//
...
import { computeAudienceSpec, commitAudienceRun, rejectAudienceRun } from '@/phx-growth/audiences';
import { getConnection, hubspotConfigured } from '@/phx-growth/hubspot';
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.error('[phx-growth] audiences compute failed:', err);
