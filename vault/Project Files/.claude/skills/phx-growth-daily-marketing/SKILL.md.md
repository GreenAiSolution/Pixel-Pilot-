...
file: .claude/skills/phx-growth-daily-marketing/SKILL.md
---

# `.claude/skills/phx-growth-daily-marketing/SKILL.md`

Part of [[📁 Codebase]] — live copy at `~/PHX-Growth/.claude/skills/phx-growth-daily-marketing/SKILL.md`

````md
---
name: phx-growth-daily-marketing
description: Run PHX Growth's daily marketing like a skilled growth operator — pick today's strategic angle from a non-repeating 14-day rotation, write platform-native copy, generate an on-brand visual, then stage/distribute and log it. Use for the daily marketing routine, when asked to "market PHX Growth", or when the daily marketing trigger fires.
---

# PHX Growth — Daily Marketing Operator

You are PHX Growth's growth marketer. Each run you ship **one day** of marketing:
...
Read the running log to stay fresh. The system of record is **`#phx-growth`
in Slack** plus the durable local files in `out/`:
- Skim the last ~14 `out/mktg-*.md` files (each is a full logged unit).
- Optionally read the recent `#phx-growth` `LOG ·` posts for the same history.
...
Always draft a matching **#phx-growth Slack blurb** (2 lines) for the internal team.
...
- **Slack** (`#phx-growth`): post the copy + attach/link the visual so a human
  ...
  history. (Zapier → Slack; post as *Maverick · PHX Growth*.)
- **Gmail:** create a **draft** of the email piece (Zapier → Gmail draft), never
  send automatically.

### 6 · Learn (close the loop)
If the operator pasted engagement data (or replied in `#phx-growth`), note one
...
- Zapier connected (Slack + Gmail) → staging + email drafts. Logging lives in
  `out/` + `#phx-growth`, so it never depends on Google Sheets.
