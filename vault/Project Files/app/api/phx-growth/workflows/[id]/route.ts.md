...
file: app/api/phx-growth/workflows/[id]/route.ts
---

# `app/api/phx-growth/workflows/[id]/route.ts`

Part of [[📁 Codebase]] — live copy at `~/PHX-Growth/app/api/phx-growth/workflows/[id]/route.ts`

**Imports** [[Project Files/phx-growth/index.ts|index.ts]]

````ts
// ─── PHX GROWTH · n8n WORKFLOW TRIGGER ──────────────────────────────────────
// POST /api/phx-growth/workflows/[id]
...
import { getWorkflow, WORKFLOWS } from '@/phx-growth';
...
export async function POST(
  ...
  try {
    const res = await fetch(`${base}${workflow.webhookPath}`, {
      ...
      body: JSON.stringify({ source: 'phx-growth', workflow: workflow.id, payload }),
    });
