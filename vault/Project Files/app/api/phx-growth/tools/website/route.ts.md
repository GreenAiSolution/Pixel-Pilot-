...
file: app/api/phx-growth/tools/website/route.ts
---

# `app/api/phx-growth/tools/website/route.ts`

Part of [[📁 Codebase]] — live copy at `~/PHX-Growth/app/api/phx-growth/tools/website/route.ts`

**Imports** [[Project Files/phx-growth/ai.ts|ai.ts]] · [[Project Files/phx-growth/store.ts|store.ts]]

````ts
// ─── PHX GROWTH · TOOL · WEBSITE CREATION & DEVELOPMENT ─────────────────────
// POST /api/phx-growth/tools/website
...
import { pushToList, set, storeIsDurable } from '@/phx-growth/store';
...
export async function POST(req: NextRequest) {
  ...
  const system =
    'You are PHX Growth, a senior conversion-focused web designer and front-end engineer. You produce clean, responsive, accessible single-page sites as self-contained HTML (inline CSS, no external assets), built to convert.';
  const prompt =
