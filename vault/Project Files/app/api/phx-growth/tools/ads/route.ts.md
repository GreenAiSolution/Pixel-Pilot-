...
file: app/api/phx-growth/tools/ads/route.ts
---

# `app/api/phx-growth/tools/ads/route.ts`

Part of [[📁 Codebase]] — live copy at `~/PHX-Growth/app/api/phx-growth/tools/ads/route.ts`

**Imports** [[Project Files/phx-growth/ai.ts|ai.ts]] · [[Project Files/phx-growth/store.ts|store.ts]]

````ts
// ─── PHX GROWTH · TOOL · PREMIUM AI ADS ─────────────────────────────────────
// POST /api/phx-growth/tools/ads
...
export async function POST(req: NextRequest) {
  ...
  const system =
    'You are PHX Growth, an elite direct-response ad copywriter and platform-policy expert. Write scroll-stopping, defensible ad copy for regulated-friendly DTC brands. No guaranteed-returns language.';
  const prompt =
