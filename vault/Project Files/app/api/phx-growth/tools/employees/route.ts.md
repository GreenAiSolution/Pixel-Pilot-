...
file: app/api/phx-growth/tools/employees/route.ts
---

# `app/api/phx-growth/tools/employees/route.ts`

Part of [[📁 Codebase]] — live copy at `~/PHX-Growth/app/api/phx-growth/tools/employees/route.ts`

**Imports** [[Project Files/phx-growth/agents.ts|agents.ts]] · [[Project Files/phx-growth/ai.ts|ai.ts]] · [[Project Files/phx-growth/store.ts|store.ts]]

````ts
// ─── PHX GROWTH · TOOL · AI EMPLOYEES ───────────────────────────────────────
// GET  /api/phx-growth/tools/employees   → the roster you can hire
// POST /api/phx-growth/tools/employees   → hire a crew + get a deployment plan
...
import { PIXEL_AGENTS, getPixelAgent } from '@/phx-growth/agents';
import { askClaudeJSON, aiConfigured, AINotConfiguredError } from '@/phx-growth/ai';
...
export async function POST(req: NextRequest) {
  ...
  const system =
    'You are PHX Growth, deploying a crew of autonomous AI operators into a business. Produce a crisp, realistic first-week deployment plan. Safe-by-default: agents stage work for approval, never auto-publish.';
  const prompt =
