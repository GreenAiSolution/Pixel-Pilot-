...
file: app/api/phx-growth/tools/brand/route.ts
---

# `app/api/phx-growth/tools/brand/route.ts`

Part of [[📁 Codebase]] — live copy at `~/PHX-Growth/app/api/phx-growth/tools/brand/route.ts`

**Imports** [[Project Files/phx-growth/ai.ts|ai.ts]] · [[Project Files/phx-growth/store.ts|store.ts]]

````ts
// ─── PHX GROWTH · TOOL · BRAND IDENTITY KIT ─────────────────────────────────
// POST /api/phx-growth/tools/brand
...
export async function POST(req: NextRequest) {
  ...
  const system =
    'You are PHX Growth, a world-class brand strategist and identity designer. You build sharp, ownable brand systems — memorable names, a defensible position, a real color system with hex codes, and a voice a team can actually write in.';
  const prompt =
