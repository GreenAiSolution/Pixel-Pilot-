...
file: app/api/phx-growth/tools/launch-plan/route.ts
---

# `app/api/phx-growth/tools/launch-plan/route.ts`

Part of [[📁 Codebase]] — live copy at `~/PHX-Growth/app/api/phx-growth/tools/launch-plan/route.ts`

**Imports** [[Project Files/phx-growth/ai.ts|ai.ts]] · [[Project Files/phx-growth/store.ts|store.ts]]

````ts
// ─── PHX GROWTH · TOOL · ZERO-TO-LIVE LAUNCH PLANNER ────────────────────────
// POST /api/phx-growth/tools/launch-plan
...
export async function POST(req: NextRequest) {
  ...
  const system =
    'You are PHX Growth, an autonomous media buyer. Turn a product or URL into a complete, defensible go-to-live plan optimizing to profit, not ROAS. No guaranteed-returns language.';
  const prompt =
