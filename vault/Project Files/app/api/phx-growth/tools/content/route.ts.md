...
file: app/api/phx-growth/tools/content/route.ts
---

# `app/api/phx-growth/tools/content/route.ts`

Part of [[📁 Codebase]] — live copy at `~/PHX-Growth/app/api/phx-growth/tools/content/route.ts`

**Imports** [[Project Files/phx-growth/ai.ts|ai.ts]] · [[Project Files/phx-growth/store.ts|store.ts]]

````ts
// ─── PHX GROWTH · TOOL · CONTENT ENGINE ─────────────────────────────────────
// POST /api/phx-growth/tools/content
...
export async function POST(req: NextRequest) {
  ...
  const system =
    'You are PHX Growth, an elite social content strategist. You write platform-native, scroll-stopping content calendars that build an audience and drive action — not generic filler.';
  const prompt =
