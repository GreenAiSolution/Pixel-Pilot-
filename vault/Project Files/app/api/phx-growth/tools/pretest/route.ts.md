...
file: app/api/phx-growth/tools/pretest/route.ts
---

# `app/api/phx-growth/tools/pretest/route.ts`

Part of [[📁 Codebase]] — live copy at `~/PHX-Growth/app/api/phx-growth/tools/pretest/route.ts`

**Imports** [[Project Files/phx-growth/ai.ts|ai.ts]] · [[Project Files/phx-growth/store.ts|store.ts]]

````ts
// ─── PHX GROWTH · TOOL · SYNTHETIC AD PRE-TESTING ───────────────────────────
// POST /api/phx-growth/tools/pretest
...
export async function POST(req: NextRequest) {
  ...
  const system =
    'You are PHX Growth\'s synthetic testing arena. You model realistic buyer personas and predict ad performance calibrated to real DTC benchmarks. Be discriminating — do not score everything highly.';
  const prompt =
