...
file: app/api/phx-growth/automations/route.ts
---

# `app/api/phx-growth/automations/route.ts`

Part of [[📁 Codebase]] — live copy at `~/PHX-Growth/app/api/phx-growth/automations/route.ts`

**Imports** [[Project Files/phx-growth/executor.ts|executor.ts]] · [[Project Files/phx-growth/quickbooks.ts|quickbooks.ts]] · [[Project Files/phx-growth/store.ts|store.ts]]

````ts
// ─── PHX GROWTH · AUTOMATIONS (persist + execute) ───────────────────────────
// POST /api/phx-growth/automations   → save a deployed automation and run it
// GET  /api/phx-growth/automations   → list what's been deployed
//
...
import { pushToList, getList, storeIsDurable } from '@/phx-growth/store';
import { triggerWorkflow, fireZapier, type Receipt } from '@/phx-growth/executor';
import { isConnected as qbConnected, companyInfo as qbCompanyInfo } from '@/phx-growth/quickbooks';
    ...
    try {
      if (await qbConnected()) {
        ...
        receipts.push({
          ...
          configured: false,
          ok: false,
          detail: 'QuickBooks not connected — connect it at /api/phx-growth/connectors/quickbooks',
        });
      }
    } catch (err) {
      receipts.push({
