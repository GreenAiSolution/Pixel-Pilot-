...
file: phx-growth/connectors.ts
---

# `phx-growth/connectors.ts`

Part of [[📁 Codebase]] — live copy at `~/PHX-Growth/phx-growth/connectors.ts`

````ts
// ─── PHX GROWTH · CONNECTORS ────────────────────────────────────────────────
// The four ad/data platforms PHX Growth flies. Each connector is a fully-typed
// OAuth definition wired to its real authorization + token endpoints, so the
// `/api/phx-growth/connectors/[provider]` route can mint a live consent URL the
...
export interface Connector {
  ...
  readonly hue: string;
  /** What PHX Growth actually does once this pipe is open. */
