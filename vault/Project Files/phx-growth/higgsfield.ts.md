...
file: phx-growth/higgsfield.ts
---

# `phx-growth/higgsfield.ts`

Part of [[📁 Codebase]] — live copy at `~/PHX-Growth/phx-growth/higgsfield.ts`

````ts
// ─── PHX GROWTH · HIGGSFIELD CREATIVE AUTOMATION ────────────────────────────
...
// a visitor drops a product, PHX Growth spins up an ad reel live.
//
...
// Genuine Higgsfield renders forged for PHX Growth — Soul v2 stills and Kling
...
/**
 * The PHX Growth brand film — a real cinematic ad (Kling v3.0 pro, 1080p): a
...
export async function generateCreative(req: CreativeRequest): Promise<CreativeJob> {
  ...
  try {
    ...
    console.error('[phx-growth] Higgsfield live render failed, using showcase:', err);
    return showcaseJob();
  }
}
````
