...
file: phx-growth/executor.ts
---

# `phx-growth/executor.ts`

Part of [[📁 Codebase]] — live copy at `~/PHX-Growth/phx-growth/executor.ts`

**Imports** [[Project Files/phx-growth/workflows.ts|workflows.ts]]
...
export async function fireZapier(event: string, payload: Record<string, unknown>): Promise<Receipt> {
  ...
  try {
    const res = await fetch(hook, {
      ...
      body: JSON.stringify({ source: 'phx-growth', event, sentAt: new Date().toISOString(), ...payload }),
    });
